...
capacity_utilization = pd.merge(
    frb_capacity, frb_utilization, on=["year", "naics_code"]
)
...
capacity_utilization["naics_code"] = (
    capacity_utilization["naics_code"].astype(str).str.extract(r"(\d+)")
)
capacity_utilization = pd.merge(
    capacity_utilization, naics_codes, on="naics_code", how="left"
)
...
capacity_utilization["mean_capacity_demeaned"] = capacity_utilization.groupby(
    "naics_code"
)["mean_capacity"].transform(lambda x: x - x.mean())
capacity_utilization["mean_utilization_demeaned"] = capacity_utilization.groupby(
    "naics_code"
)["mean_utilization"].transform(lambda x: x - x.mean())
...
usetables_path = os.path.join(
    data_folder, "raw", "bea", "IOUse_Before_Redefinitions_PRO_1997-2023_Summary.xlsx"
)
...
for year in range(1968, 2024):
    ...
    if year <= 1996:
        ...
        GFGSCL = (
            GFGSCL["National defense: Consumption expenditures"][
                GFGSCL["IOCode"] == "GFG"
            ]
            / GFGSCL["Total Commodity Output"][GFGSCL["IOCode"] == "GFG"]
        )
    ...
    UCS = (
        np.linalg.inv(np.eye(II) - C_p) * tmp
    ).T  # hademard. we transpose because source and dest are flipped vs above
    DCS = IO / (total - VA)
    ...
    IOT_df = pd.DataFrame(
        IO.T, columns=names, index=names
    )  # later, will use in i's sales shares to j
    totalsq = np.outer(total, np.ones(II))
    total_df = pd.DataFrame(
        totalsq, columns=names, index=names
    )  # later, will use in i's sales shares to j
    ...
    IOT_df.columns = [
        ...
        "naics_source",
        "IOT",
    ]  # flip indices to get Sji, i's sales shares to j, later
    IOT_df["IOT"][IOT_df["naics_source"] == "GFG"] = (
        IOT_df["IOT"][IOT_df["naics_source"] == "GFG"] * GFGSCL
    )  # rescale by GFGSCL
...
post_xls_boehm = os.path.join(data_folder, "raw", "bea", "use_tables_BOEHM.xlsx")
...
graphdf = pd.DataFrame(
    graphdf, columns=["year", "mean_diff", "median_diff", "std_diff"]
)
...
plt.plot(
    graphdf["year"],
    graphdf["mean_diff"],
    label="Mean Absolute Percent Difference",
    color="blue",
)
plt.plot(
    graphdf["year"],
    graphdf["median_diff"],
    label="Median Absolute Percent Difference",
    color="orange",
)
...
plt.plot(
    graphdf["year"],
    graphdf["std_diff"],
    label="Standard Deviation of Absolute Percent Difference",
    color="green",
)
...
IOdiff_long = IOdiff_long.melt(
    var_name="naics", value_name="IO_diff"
)  # note: this label not =naics
...
quantities = quantities.melt(
    id_vars=["naics_code", "IO_Code"], var_name="year", value_name="qty_index"
)
...
# check description column for duplicates
duplicates_hist = quantities_hist[
    quantities_hist.duplicated(subset=["Description"], keep=False)
]
...
# reshape long to time series
quantities_hist = quantities_hist.melt(
    id_vars=["naics_code", "IO_Code"], var_name="year", value_name="qty_index"
)
...
intermediate_ts_path = os.path.join(
    data_folder, "raw", "bea", "intermediate_ts_BOEHM.dta"
)
...
intermediate_ts = pd.merge(
    intermediate_ts, intermediate_ts_ind, on=["IO_Code"], how="left"
)
intermediate_ts["qty_index_BOEHM"] = (
    intermediate_ts["quantity_index"] / intermediate_ts["qty_2017"] * 100
)
...
quantities_sam = pd.merge(
    intermediate_ts, quantities_sam, on=["IO_Code", "year"], how="inner"
)

quantities_sam["BOEHM_min_sam"] = (
    (quantities_sam["qty_index_BOEHM"] - quantities_sam["qty_index"])
    / quantities_sam["qty_index"]
    * 100
)
...
quantities_sam["BOEHM_min_sam_abs"] = np.abs(
    quantities_sam["BOEHM_min_sam"].astype(float)
)
# collapse by year
quantities_sam = quantities_sam.groupby("year", as_index=False)[
    ["BOEHM_min_sam", "BOEHM_min_sam_abs"]
].agg(["mean", "median", "std"])
...
plt.plot(
    quantities_sam["year"],
    quantities_sam["BOEHM_min_sam"]["mean"],
    label="Mean Percent Difference",
    color="blue",
)
plt.plot(
    quantities_sam["year"],
    quantities_sam["BOEHM_min_sam"]["median"],
    label="Median Percent Difference",
    color="orange",
)
plt.plot(
    quantities_sam["year"],
    quantities_sam["BOEHM_min_sam"]["std"],
    label="Standard Deviation of Percent Difference",
    color="green",
)
...
plt.plot(
    quantities_sam["year"],
    quantities_sam["BOEHM_min_sam_abs"]["mean"],
    label="Mean Absolute Percent Difference",
    color="blue",
)
plt.plot(
    quantities_sam["year"],
    quantities_sam["BOEHM_min_sam_abs"]["median"],
    label="Median Absolute Percent Difference",
    color="orange",
)
plt.plot(
    quantities_sam["year"],
    quantities_sam["BOEHM_min_sam_abs"]["std"],
    label="Standard Deviation of Absolute Percent Difference",
    color="green",
)
...
shea_full = pd.merge(
    cost_shares, demand_shares, on=["naics_source", "naics_dest", "year"], how="left"
)
...
shea_C_ji.rename(
    columns={"naics_source": "naics_dest", "naics_dest": "naics_source"}, inplace=True
)
...
shea_full = pd.merge(
    shea_full, shea_C_ji, on=["naics_source", "naics_dest", "year"], how="left"
)
...
shea_full["stat2"] = np.minimum(shea_full["DDS"], shea_full["UDS"]) / np.maximum.reduce(
    [shea_full["DCS"], shea_full["UCS"], shea_full["DCS_ji"], shea_full["UCS_ji"]]
)
...
shea_full["LJShea"] = (shea_full["stat2"] > shea_threshold).astype(
    int
)  # Defining JShea
...
shea_full["naics_source_temp"] = shea_full["naics_source"].apply(
    lambda x: split_mapping.get(x, [x])
)
...
shea_full["sales_sh"] = (
    shea_full["IOT"] / shea_full["total"]
)  # sales shares sj,i,t of industry i to buyer j; adj as in a01_construct_sample.do
shea_full["exp_sh"] = (
    shea_full["exp"] / shea_full["total"]
)  # export shares sj,i,t of industry i to buyer j
...
shea_full.rename(
    columns={"sales_sh": "Lsales_sh"}, inplace=True
)  # since we alr did year = year + 1
...
quantities_forshea["Dln_M"] = (
    quantities_forshea.groupby("naics_dest")["qty_index"].pct_change() * 100
)
...
shea_calc = pd.merge(
    shea_full, quantities_forshea, on=["naics_dest", "year"], how="inner"
)

shea_calc["Dln_M_shea_inst2"] = (
    shea_calc["Lsales_sh"] * shea_calc["Dln_M"] * shea_calc["LJShea"]
)
# collapse (sum) by naics_source and year
shea_calc["total_by_naicsyear"] = 1
shea_calc = shea_calc.groupby(["naics_source", "year"], as_index=False).agg(
    {"Dln_M_shea_inst2": "sum", "LJShea": "sum", "total_by_naicsyear": "sum"}
...
shJS_by_i = shea_calc.groupby(["naics_source"], as_index=False).agg(
    {"LJShea": "sum", "total_by_naicsyear": "sum"}
)
shJS_by_i["LJShea"] = (
    shJS_by_i["LJShea"] / shJS_by_i["total_by_naicsyear"]
...
exports.rename(
    columns={"exp_share": "Lexp_share", "naics": "naics_source"}, inplace=True
)
# year = year + 1
